...
CREATE TABLE `note_user` (
  ...
  `type` int(255) NOT NULL DEFAULT '1' COMMENT '用户类型 user admin',
  `avatar` varchar(255) NOT NULL COMMENT '用户头像',
  `status` int(255) NOT NULL COMMENT '用户状态',
  PRIMARY KEY (`uid`)
...
CREATE TABLE `user` (
	...
	`type`  varchar(255)  DEFAULT 'USER' COMMENT '用户类型 user admin' ,
	`avatar`  varchar(255) NOT NULL COMMENT '用户头像' ,
...
CREATE TABLE `note` (
  ...
  `updatedUserId` varchar(200) DEFAULT NULL,
  PRIMARY KEY (`note_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `notebook` (
  `note_book_id` char(12) NOT NULL,
  `userId` varchar(200) DEFAULT NULL,
	`title` varchar(200) DEFAULT NULL,
  `urlTitle` varchar(200) DEFAULT NULL,
  `seq` int(11) DEFAULT '100',
  `isDeleted` varchar(200) DEFAULT 'false',
